...
class GlobalFineTuner:
    ...
    def fine_tune(self, target_date, is_debug=False):
        ...
        for epoch in range(epochs):
            ...
            logger.info(f"Epoch {epoch+1}/{epochs} - 训练损失: {avg_train_loss:.4f},训练真实损失: {train_true_loss:.4f},训练集RMSE: {train_rmse:.2f},训练集R2: {r2_score(all_val_targets_inv, all_val_outputs_inv):.4f}, \n"
                        f"验证损失: {avg_val_loss:.4f}, 验证集RMSE: {val_rmse:.2f}, 验证集R2: {val_r2:.4f}, \n"
...
if __name__ == "__main__":
    ...
    try:
        ...
        for date in pd.date_range(start='2018-01-01', end='2018-12-31', freq='D'):
            ...
            finetune_model_path_data = finetuner.fine_tune(date)
