...
# 配置日志：同时输出到文件和控制台
log_dir = os.path.join(os.path.dirname(__file__), '../output/data_loder')
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, 'data_loder.log')
...
class DataLoader:
    ...
    def _load_dynamic_data(self, file_path, var_name, shape, nodata_value, data_type=None):
        ...
        try:
            ...
            if ds is not None and var_name in ds.variables:
                ...
                # 针对 cldas，>400 也视为无效
                if data_type == "cldas":
                    data[data > 330 and data < 240] = np.nan'''
            ...
            try:
                ...
                if var_name=='TG'and cur_max >= 320:              
    ...
    def load_data(self, target_date, is_global, is_pretrain):
        ...
        if is_pretrain:
            ...
            if target_date.month == 12:
                end_date = datetime(target_date.year + 1, 1, 1) - timedelta(hours=1)
            else:              
                end_date = start_date.replace(month=start_date.month % 12 + 1, day=1) - timedelta(hours=1)
        else:
            # 微调时间窗口为目标日期前后共30天，并包含最后一天的所有小时
            start_date = target_date - timedelta(days=15)#15
            end_date = target_date + timedelta(days=14, hours=23)#14
    ...
    def load_data_for_interpolation(self, target_date, target_hour):
            """
            专门用于插补的数据加载方法，只加载指定日期和小时的数据
            ...
            logger.info(f"加载目标时间 {target_date.strftime('%Y-%m-%d')} {target_hour:02d}:00 的插补数据")
            
            # 构建目标时间
            target_datetime = datetime.combine(target_date, datetime.min.time()) + timedelta(hours=target_hour)
            
            # 查找目标小时的MODIS文件
            start_time = target_datetime - timedelta(minutes=30)  # 允许30分钟的时间窗口
            end_time = target_datetime + timedelta(minutes=30)
            
            modis_files = self.get_file_list(paths['modis'], start_time, end_time, 'modis')
            
            if not modis_files:
                logger.error(f"未找到目标时间 {target_datetime} 的MODIS文件")
                return None
            
            # 选择最接近目标时间的文件
            target_modis_file = self.find_closest_file(modis_files, target_datetime, timedelta(hours=1))
            if not target_modis_file:
                logger.error(f"未找到 {target_datetime} 附近的MODIS文件")
                return None
            
            logger.info(f"找到MODIS文件: {os.path.basename(target_modis_file)}")
            
            # 加载MODIS LST数据
            lst_data = self._load_dynamic_data(
                target_modis_file,
                self.var_names['modis'],
                shape,
                self.nodata_values.get('modis'),
                data_type='modis'
            )
            
            if lst_data is None:
                logger.error(f"MODIS数据加载失败: {target_modis_file}")
                return None
            ...
            for var_name, data in static_data.items():
                hourly_features[var_name] = data
            
            # 添加空间特征
            hourly_features['lon'] = lon_lat[:, 0]
            hourly_features['lat'] = lon_lat[:, 1]
            lon_rad = np.radians(hourly_features['lon'])
            lat_rad = np.radians(hourly_features['lat'])
            hourly_features['lon_sin'] = np.sin(lon_rad)
            hourly_features['lon_cos'] = np.cos(lon_rad)
            ...
            # 添加辅助数据（变化较慢的数据）
            auxiliary_data = {}
            target_noon = target_datetime.replace(hour=12)  # 使用中午时间查找最接近的辅助数据
            
            # 获取辅助数据文件列表（扩大搜索范围以确保找到数据）
            aux_start_time = target_datetime - timedelta(hours=12)
            aux_end_time = target_datetime + timedelta(hours=12)
            
            max_deltas = {
                'era5_1km': timedelta(hours=1),    # ERA5 - 每小时变化，但允许6小时内的数据
                'smap_1km': timedelta(hours=3),   # SMAP - 变化较慢，允许12小时内的数据  
                'ndvi_1km': timedelta(days=16),    # NDVI - 变化很慢，允许16天内的数据
            }
            
            # 加载所有辅助数据类型
            for var_type in ['era5_1km', 'smap_1km', 'ndvi_1km']:
                if var_type not in paths:
                    logger.warning(f"路径配置中缺少 {var_type}")
                    continue
                    
                aux_files = self.get_file_list(paths[var_type], aux_start_time, aux_end_time, var_type)
                if aux_files:
                    # 找到最接近目标时间的文件
                    closest_file = self.find_closest_file(aux_files, target_datetime, max_deltas[var_type])
                    if closest_file:
                        var_names_list = self.var_names.get(var_type.replace('_1km', ''), [])
                        if isinstance(var_names_list, str): 
                            var_names_list = [var_names_list]
                        for var_name in var_names_list:
                            data = self._load_dynamic_data(
                                closest_file, 
                                var_name, 
                                shape, 
                                self.nodata_values.get(var_type.replace('_1km', '')), 
                                data_type=var_type
                            )
                            if data is not None:
                                auxiliary_data[var_name] = data
                                logger.info(f"加载辅助数据 {var_name} 从文件 {os.path.basename(closest_file)}")
                    else:
                        logger.warning(f"未找到 {var_type} 的最接近文件")
                else:
                    logger.warning(f"未找到 {var_type} 的任何文件")
            
            # 将辅助数据添加到小时特征中
            for var_name, data in auxiliary_data.items():
                hourly_features[var_name] = data
            
            # 添加时间特征
            #hourly_features['month'] = np.full(total_pixels, target_datetime.month)
            hourly_features['day'] = np.full(total_pixels, target_datetime.day)
            hourly_features['hour'] = np.full(total_pixels, target_datetime.hour)
            hourly_features['doy'] = np.full(total_pixels, target_datetime.timetuple().tm_yday)
            
            # 检查关键特征是否已加载
            expected_features = ['t2m', 'ssrd', 'strd', 'rh', 'd2m', 'vpd', 'sm_surface_wetness', '_1_km_16_days_NDVI']
            missing_features = [feature for feature in expected_features if feature not in hourly_features]
            
            if missing_features:
                logger.warning(f"缺少以下关键特征: {missing_features}")
            
            # 统计特征信息
            num_features = len([k for k in hourly_features.keys() if k not in ['clcd', 'sm_surface_wetness', 'day', 'hour', 'doy', 
                                                                               'lon_sin', 'lon_cos', 'lat_sin', 'lat_cos']]) #  'month',
            cat_features = len([k for k in hourly_features.keys() if k in ['clcd', 'sm_surface_wetness']])
            
            logger.info(f"小时 {target_hour:02d} 特征统计: 数值特征={num_features}, 分类特征={cat_features}, "
                    f"总特征={len(hourly_features)}")
            
            # 返回简化的数据结构，直接包含该小时的数据
            return {
